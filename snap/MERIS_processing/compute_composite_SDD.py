...
def computeCompositeSDD(compositeProduct):
    ...
    SDD_bandnames=[var for var in bandnames if var.split('_')[0]=='secchi' and len(var.split('_'))>2 ]

    # Apply bandmaths in for loop
    newBandName = 'composite_secchi_depth_0'
    datatype = 'float32'
    ...
    for comp_ind in range(0,len(SDD_bandnames)):
        ...
        if comp_ind == len(SDD_bandnames)-1:
            newBandName = 'composite_secchi_depth_final'
        else:
            newBandName = 'composite_secchi_depth_'+str(comp_ind+1)
        datatype = 'float32'
        expression = 'if (not nan(composite_secchi_depth_'+str(comp_ind)+') and not nan('+SDD_bandnames[comp_ind]+')) then max(composite_secchi_depth_'+str(comp_ind)+','+SDD_bandnames[comp_ind]+')'+' else (if not nan(composite_secchi_depth_'+str(comp_ind)+') then composite_secchi_depth_'+str(comp_ind)+' else '+SDD_bandnames[comp_ind]+')'
