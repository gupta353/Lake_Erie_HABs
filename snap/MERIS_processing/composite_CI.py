...
# read Lake Erie mask product
direc1 = 'D:/Research/EPA_Project/Lake_Erie_HAB/Data/remote_sensing_data/Lake_Erie_mask'
fname = 'LE_mask_sentinel.dim'
filename = direc1+'/'+fname
LE_mask_prd = ProductIO.readProduct(filename);
...
# Create a product containing CI band (filled with) with maximum size out of the products to be listed in the same time-window
for tw_ind in range(0,num_time_windows):
    ...
    if len(ind) != 0:
        ...
        # resample and merge LE mask band (Mosaicing could not be done as it thrown an unknown error)
        param=HashMap()
        param.put('targetWidth',widthp)
        param.put('targetHeight',heightp)
        param.put('upsampling','Nearest')
        param.put('downsampling','Mean')
        prod_tmp = GPF.createProduct('Resample', param, LE_mask_prd)
        ...
        for fname in fname_tw:
            ...
            newBandName = 'CI_'+str(count)
            datatype = 'float32'
            expression = 'CI_'+str(count)+'_count ==1?CI_'+str(count)+':NaN'
            noDataVal = 'NaN'
